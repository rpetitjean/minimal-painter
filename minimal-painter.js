...
AFRAME.registerComponent('paint-tool-reset', {
  ...
  assignTools(side, force = false) {
    ...
    // Preserve any existing touch-button-colors configs before cleanup
    ...
    [ this.leftHand, this.rightHand ].forEach(hand => {
      ...
      // NEW: ensure only the painter will have colored buttons
    ...
    // NEW: re-attach touch-button-colors ONLY to the painter.
    // Prefer painter's own previous config; else borrow palette's; else defaults.
    const cfgToUse =
...
AFRAME.registerComponent('hand-swapper', {
  ...
  activate(side) {
    ...
    // Preserve configs before cleanup
    const leftCfg  = this.leftHand .getAttribute('touch-button-colors');
    const rightCfg = this.rightHand.getAttribute('touch-button-colors');
    ...
    [this.leftHand, this.rightHand].forEach(h => {
      ...
      // NEW: strip colors; we’ll re-attach to the painter below
    ...
    // NEW: attach touch-button-colors ONLY to painter (preserve per-hand config)
    const cfgToUse =
      (painter === this.leftHand ? leftCfg : rightCfg) ||
      (painter === this.leftHand ? rightCfg : leftCfg) || '';
    painter.setAttribute('touch-button-colors', cfgToUse);
...
// HAND-SIDE-HINTS — one square hint per hand, not attached to controllers
AFRAME.registerComponent('hand-side-hints', {
  schema: {
    leftSel:  { default: '#left-hand'  },
    rightSel: { default: '#right-hand' },

    // visual
    size:         { default: 0.028 },          // square (m)
    tint:         { default: '#111' },
    opacity:      { default: 0.9 },
    imgLeft:      { default: '' },             // optional image (selector or URL)
    imgRight:     { default: '' },
    billboard:    { default: true },           // face camera

    // placement relative to each hand (local space)
    outerOffset:  { default: 0.045 },          // ±X (right:+, left:-)
    raise:        { default: 0.012 },          // +Y
    forward:      { default: 0.000 },          // +Z

    // visibility rules
    showOnlyInZone: { default: true },         // hide when outside painting area
    showOnlyPainter:{ default: false }         // if true, show only active-brush hand
  },

  init() {
    this.left  = document.querySelector(this.data.leftSel);
    this.right = document.querySelector(this.data.rightSel);

    this.hintL = this._makeHint(this.data.imgLeft);
    this.hintR = this._makeHint(this.data.imgRight);

    // start hidden; we'll manage visibility in tick()
    this.hintL.object3D.visible = false;
    this.hintR.object3D.visible = false;
  },

  remove() {
    if (this.hintL) this.hintL.remove();
    if (this.hintR) this.hintR.remove();
  },
  ...
  tick() {
    ...
    const inZone = !this.data.showOnlyInZone
      || (this.el.components['painting-area-controller']?.inside === true);
    ...
    this._updateForHand({
      ...
      hint: this.hintL,
      sign: -1,                               // left side → -X
      visible: inZone && (!this.data.showOnlyPainter || painter === this.left),
      cam
    });
    ...
    this._updateForHand({
      ...
      hint: this.hintR,
      sign: +1,                               // right side → +X
      visible: inZone && (!this.data.showOnlyPainter || painter === this.right),
      cam
    });
  },
  ...
  _makeHint(src) {
    const p = document.createElement('a-plane');
    p.setAttribute('width',  this.data.size);
    p.setAttribute('height', this.data.size);
    const mat = src
      ? `src:${src}; side:double; transparent:true`
      : `color:${this.data.tint}; opacity:${this.data.opacity}; transparent:true; side:double`;
    p.setAttribute('material', mat);
    // make it render on top if very close
    p.setAttribute('render-order', '10');
    this.el.appendChild(p); // attach under the RIG (not the controller)
    return p;
  },

  _updateForHand({hand, hint, sign, visible, cam}) {
    if (!hand?.object3D || !hint?.object3D) return;

    // Toggle visibility
    hint.object3D.visible = !!visible;
    if (!visible) return;

    // Compute world position from hand local offset
    const local = new THREE.Vector3(
      sign * this.data.outerOffset,
      this.data.raise,
      this.data.forward
    );
    const worldPos = hand.object3D.localToWorld(local);
    hint.object3D.position.copy(worldPos);

    // Orient
    if (this.data.billboard) {
      const camPos = new THREE.Vector3();
      cam.object3D.getWorldPosition(camPos);
      hint.object3D.lookAt(camPos);
    } else {
      // Face outward from hand along ±X
      const outwardLocal = new THREE.Vector3(sign, 0, 0);
      const outwardWorld = outwardLocal.applyQuaternion(hand.object3D.getWorldQuaternion(new THREE.Quaternion()));
      hint.object3D.lookAt(worldPos.clone().add(outwardWorld));
    }
  }
});
